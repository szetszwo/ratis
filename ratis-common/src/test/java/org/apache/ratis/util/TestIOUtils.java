...
public class TestIOUtils extends BaseTest {
  ...
  public void testReadObjectRejectsDisallowedClass() throws IOException {
    // BigDecimal lives in java.math.*, which is outside the allowlist.
    // Serialize it directly so we know the bytes are well-formed; the filter
    // must reject on read.
    final ByteArrayOutputStream bout = new ByteArrayOutputStream();
    try (ObjectOutputStream oout = new ObjectOutputStream(bout)) {
      oout.writeObject(new BigDecimal("42.0"));
    }
    final byte[] bytes = bout.toByteArray();

    final IllegalStateException ise = Assertions.assertThrows(IllegalStateException.class,
        () -> IOUtils.bytes2Object(bytes, Object.class));
